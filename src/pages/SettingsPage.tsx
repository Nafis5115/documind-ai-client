...
const SettingsPage = () => {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 lg:p-8  space-y-8">
      ...
      {/* Profile card */}
      <div
        className="glass-subtle rounded-2xl p-6 flex items-center gap-5 animate-fade-in"
      ...
      >
        ...
        <div className="flex-1">
          ...
          <span className="inline-block mt-2 text-xs px-3 py-1 rounded-full gradient-bg-primary text-primary-foreground font-medium">
            Pro Plan
          </span>
        </div>
        <button
          onClick={() => setSubPage("profile")}
          className="px-4 py-2 rounded-xl border border-border/50 text-sm text-muted-foreground hover:text-foreground hover:border-primary/40 transition-all"
        >
          Edit Profile
        </button>
      </div>
          ...
          <div className="glass-subtle rounded-2xl overflow-hidden divide-y divide-border/30">
            {section.items.map((item) => (
              <button
                ...
                className="w-full flex items-center gap-4 p-4 hover:bg-muted/30 transition-all text-left group"
              >
      ...
      {/* Danger zone */}
      <div
        className="glass-subtle rounded-2xl p-6 border border-destructive/20 animate-fade-in"
