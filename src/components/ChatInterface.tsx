...
import { Send, Paperclip, Sparkles, Zap, ListChecks, Brain, FileText } from 'lucide-react';
...
const suggestedPrompts = [
  { icon: Sparkles, label: 'Summarize', prompt: 'Summarize the key points of this document' },
  { icon: Brain, label: 'Explain simply', prompt: 'Explain this document in simple terms' },
  { icon: ListChecks, label: 'Key points', prompt: 'Extract the key points and action items' },
  { icon: Zap, label: 'Extract insights', prompt: 'What are the most important insights?' },
];
...
const ChatInterface = ({ chat, onSendMessage }: ChatInterfaceProps) => {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
        ...
        {isEmpty ? (
          <div className="flex-1 flex flex-col items-center justify-center h-full min-h-[50vh]">
            ...
            <p className="text-muted-foreground text-sm mb-8 text-center max-w-md">
              Ask me anything about your documents. I can summarize, explain, extract insights, and more.
            </p>
          ...
          <>
            {chat.messages.map((msg, i) => (
              <div key={msg.id} className={`flex items-start gap-3 animate-fade-in ${msg.role === 'user' ? 'justify-end' : ''}`} style={{ animationDelay: `${i * 0.05}s` }}>
                {msg.role === 'ai' && (
                  <div className="w-8 h-8 rounded-xl gradient-bg-accent flex items-center justify-center flex-shrink-0 mt-0.5">
                    <Sparkles className="w-4 h-4 text-accent-foreground" />
                  </div>
                )}
                <div className={`max-w-[75%] ${msg.role === 'user' ? 'chat-bubble-user px-5 py-3' : 'chat-bubble-ai px-5 py-4'}`}>
                  ...
                  {msg.role === 'ai' && (
                    <div className="mt-3 flex items-center gap-2">
                      {['Summarize', 'Simplify', 'Extract insights'].map(action => (
                        <button key={action} className="text-xs px-2.5 py-1 rounded-lg border border-border/50 text-muted-foreground hover:text-foreground hover:border-primary/40 transition-all">
                          {action}
                        </button>
                      ))}
                    </div>
      ...
      <div className="px-4 lg:px-8 pb-4 pt-2">
        ...
        <p className="text-center text-xs text-muted-foreground/40 mt-2">DocuMind AI may produce inaccurate information. Always verify important facts.</p>
      </div>
    </div>
  );
};
